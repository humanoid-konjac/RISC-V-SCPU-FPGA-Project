...
7. 在流水线CPU上实现复杂的应用。（最终目标）
   ...
   当前应用为支持 36 个离线验证关卡、三档难度、完整撤销和菜单的倒水排序游戏。C 程序负责规则、关卡目录和历史，Verilog 负责键盘中断请求、MMIO 与 VGA 字符/试管实时渲染，不使用完整帧缓冲。开发子进度如下：
   ...
   - [x] Step 6.2：每档 12 个离线随机候选、BFS 验证并按最短解筛选的关卡目录。（2026-07-10 全部 36 关通过）
   - [x] Step 6.3：1 字节/步、最多 2048 步的完整撤销，支持通关后撤销。（2026-07-10 主机与整机仿真通过）
   - [x] Step 6.4：上下选难度、左右选关、数字输入 1～12、重开当前关和返回菜单。（2026-07-10 RTL/真实 SCPU 仿真通过）
   - [x] Step 6.5：英文 5×7 字库、状态/控制提示、动态居中和通关文字。（2026-07-10 关键像素与顶层展开通过，待上板）
   - [x] Step 6.6：加入 `tools/generate_levels.py`、压缩关卡目录和 `LEVEL_GENERATION.md`，构建时检查目录可复现。（2026-07-10 通过）
   - [x] Step 7：将游戏键盘输入从 MMIO 轮询改为机器模式中断；trap 入口保存/恢复全部整数寄存器，C ISR 处理事件并 ACK，主循环不再读取键盘状态。（2026-07-11 真实 SCPU 整机仿真通过，待上板）

   当前控制：菜单用上下/W/S 选难度、左右/A/D 选关、数字键输入 1～12、Backspace 删除、Enter 开始；游戏用 U 撤销、R 重开当前关、M 返回，原移动/倾倒/Esc 保留。每完成一个 Step，必须同步更新本清单、`README.md`、Vivado 导入清单和验证结果。
...
- `key_ready` 上升沿与原 `counter0_OUT` 上升沿在顶层合并到 CPU 的单个 `INT` 输入。游戏固件开启 `mstatus.MIE`/`mie.MTIE`，由 trap 入口调用 C 键盘 ISR；不得重新改回主循环轮询。
...
- 游戏画面按难度显示居中的 6/7/8 根试管，并带英文菜单、关卡号/步数/控制提示、第七种颜色、通关文字和闪烁边框；状态在 `COMMIT` 后于下一 `frame_tick` 原子切换。
...
完整裸机固件位于 `software/water_sort/fpga/`。2026-07-11 中断驱动构建结果为 `.text` 3204 字节、`.rodata` 576 字节、`.bss` 2112 字节，保留至少 512 字节栈；真实 `SCPU` 已通过键盘中断进入、非法关卡拒绝、LEVEL 12、困难模式、倾倒、撤销、重开和返回菜单整机仿真，COE 已纳入仓库，仍需上板确认。
