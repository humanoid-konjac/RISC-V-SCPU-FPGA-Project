...
7. 在流水线CPU上实现复杂的应用。（最终目标）
   ...
   当前应用为支持 36 个离线验证关卡、三档难度、完整撤销和菜单的倒水排序游戏。C 程序负责规则、关卡目录和历史，Verilog 负责按键事件、MMIO 与 VGA 字符/试管实时渲染，不使用完整帧缓冲。开发子进度如下：
...
完整裸机固件位于 `software/water_sort/fpga/`。2026-07-10 多关卡构建结果为 `.text` 2888 字节、`.rodata` 576 字节、`.bss` 2112 字节，保留至少 512 字节栈；真实 `SCPU` 已通过非法关卡拒绝、LEVEL 12、困难模式、倾倒、撤销、重开和返回菜单整机仿真，COE 已纳入仓库，仍需上板确认。
