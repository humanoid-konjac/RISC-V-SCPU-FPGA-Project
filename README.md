...
Step 1 的主机端源代码和测试位于 `software/water_sort/`；Step 2/3 裸机诊断固件位于 `software/water_sort/fpga/`。2026-07-10 已通过主机 C 测试、新增 MMIO/VGA 模块测试、原 PS/2/VGA/VGA 时序回归、VGA 输出毛刺隔离测试、`top` 展开和 GNU RV32I 固件构建。当前指令段为 268 字节，数据初始化镜像为一个零字，生成的 `coe/game_phase3_i.coe` 和 `coe/game_phase3_d.coe` 已纳入仓库。
